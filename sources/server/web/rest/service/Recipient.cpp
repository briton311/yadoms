...
namespace web
{
   namespace rest
   {
      namespace service
      {
         ...
         shared::CDataContainer CRecipient::getOneRecipient(const std::vector<std::string>& parameters, const std::string& requestContent) const
         {
            ...
            boost::shared_ptr<dbCommon::entities::CRecipient> recipient = m_dataProvider->getRecipientRequester()->getRecipient(recipientId);
            return CResult::GenerateSuccess(recipient);
         }

         shared::CDataContainer CRecipient::getAllRecipientsByField(const std::vector<std::string>& parameters, const std::string& requestContent) const
         {
            if (parameters.size() != 3)
               return CResult::GenerateError("Invalid parameter count (need name of the field in url)");

            std::string fieldName = parameters[2];
            std::vector<boost::shared_ptr<dbCommon::entities::CRecipientField>> fields = m_dataProvider->getRecipientRequester()->getFieldsByName(fieldName);
            shared::CDataContainer collection;
            collection.set(m_restFieldKeyword, fields);
            return CResult::GenerateSuccess(collection);
         }

         shared::CDataContainer CRecipient::getAllRecipients(const std::vector<std::string>& parameters, const std::string& requestContent) const
         {
            std::vector<boost::shared_ptr<dbCommon::entities::CRecipient>> dvList = m_dataProvider->getRecipientRequester()->getRecipients();
            shared::CDataContainer collection;
            collection.set(getRestKeyword(), dvList);
            return CResult::GenerateSuccess(collection);
         }

         shared::CDataContainer CRecipient::getAllRecipientFields(const std::vector<std::string>& parameters, const std::string& requestContent) const
         {
            std::vector<boost::shared_ptr<dbCommon::entities::CRecipientField>> dvList = m_dataProvider->getRecipientRequester()->getFields();
            shared::CDataContainer collection;
            collection.set(m_restFieldKeyword, dvList);
            return CResult::GenerateSuccess(collection);
         }

         shared::CDataContainer CRecipient::addRecipient(const std::vector<std::string>& parameters, const std::string& requestContent) const
         {
            try
            {
               dbCommon::entities::CRecipient toAdd;
               toAdd.fillFromSerializedString(requestContent);
               boost::shared_ptr<dbCommon::entities::CRecipient> recipientFound = m_dataProvider->getRecipientRequester()->addRecipient(toAdd);
         ...
         shared::CDataContainer CRecipient::updateRecipient(const std::vector<std::string>& parameters, const std::string& requestContent) const
         {
            if (parameters.size() <= 1)
            ...
            try
            {
               dbCommon::entities::CRecipient toUpdate;
               ...
               boost::shared_ptr<dbCommon::entities::CRecipient> recipientFound = m_dataProvider->getRecipientRequester()->updateRecipient(toUpdate);
               return CResult::GenerateSuccess(recipientFound);
            }
            catch (std::exception& ex)
            {
         ...
         shared::CDataContainer CRecipient::removeOneRecipient(const std::vector<std::string>& parameters, const std::string& requestContent) const
         {
            try
            {
               if (parameters.size() > 1)
         ...
         shared::CDataContainer CRecipient::removeAllRecipients(const std::vector<std::string>& parameters, const std::string& requestContent) const
         {
            try
            {
