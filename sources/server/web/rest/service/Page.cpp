...
namespace web
{
   namespace rest
   {
      namespace service
      {
         ...
         {
            boost::shared_ptr<dbCommon::ITransactionalProvider> pTransactionalEngine = m_dataProvider->getTransactionalEngine();
         ...
         shared::CDataContainer CPage::getOnePage(const std::vector<std::string>& parameters, const std::string& requestContent)
         {
            if (parameters.size() > 1)
            {
               ...
               boost::shared_ptr<dbCommon::entities::CPage> pageFound = m_dataProvider->getPageRequester()->getPage(pageId);
         ...
         {
            std::vector<boost::shared_ptr<dbCommon::entities::CPage>> pageList = m_dataProvider->getPageRequester()->getPages();
         ...
         shared::CDataContainer CPage::getPageWidget(const std::vector<std::string>& parameters, const std::string& requestContent)
         {
            ...
            if (parameters.size() > 1)
            {
               ...
               std::vector<boost::shared_ptr<dbCommon::entities::CWidget>> widgetList = m_dataProvider->getWidgetRequester()->getWidgetsForPage(boost::lexical_cast<int>(pageId));
               shared::CDataContainer collection;
         ...
         shared::CDataContainer CPage::addPage(const std::vector<std::string>& parameters, const std::string& requestContent)
         {
            try
            {
               dbCommon::entities::CPage pageToAdd;
               ...
               boost::shared_ptr<dbCommon::entities::CPage> pageFound = m_dataProvider->getPageRequester()->getPage(idCreated);
               return CResult::GenerateSuccess(pageFound);
            }
            catch (std::exception& ex)
            {
         ...
         shared::CDataContainer CPage::updatePage(const std::vector<std::string>& parameters, const std::string& requestContent)
         {
            try
            {
               if (parameters.size() > 1)
               {
                  ...
                  dbCommon::entities::CPage pageToReplace;
         ...
         shared::CDataContainer CPage::updateAllPages(const std::vector<std::string>& parameters, const std::string& requestContent)
         {
            try
            {
               ...
               std::vector<boost::shared_ptr<dbCommon::entities::CPage>> pagesToUpdate = shared::CDataContainer(requestContent).get<std::vector<boost::shared_ptr<dbCommon::entities::CPage>>>("");
               for (std::vector<boost::shared_ptr<dbCommon::entities::CPage>>::iterator i = pagesToUpdate.begin(); i != pagesToUpdate.end(); ++i)
               {
                  m_dataProvider->getPageRequester()->addPage(*i->get());
               }

               std::vector<boost::shared_ptr<dbCommon::entities::CPage>> allPages = m_dataProvider->getPageRequester()->getPages();
               shared::CDataContainer collection;
         ...
         shared::CDataContainer CPage::replaceAllWidgetsForPage(const std::vector<std::string>& parameters, const std::string& requestContent)
         {
            try
            {
               if (parameters.size() > 1)
               {
                  ...
                  //crreate all
                  std::vector<boost::shared_ptr<dbCommon::entities::CWidget>> widgetsToAdd = shared::CDataContainer(requestContent).get<std::vector<boost::shared_ptr<dbCommon::entities::CWidget>>>("");
