...
namespace web
{
   namespace rest
   {
      namespace service
      {
         ...
         shared::CDataContainer CTask::getOneTask(const std::vector<std::string>& parameters, const std::string& requestContent) const
         {
            if (parameters.size() > 1)
            {
               auto taskUid = parameters[1];
               auto taskFound = m_taskManager->getTask(taskUid);
         ...
         shared::CDataContainer CTask::getAllTasks(const std::vector<std::string>& parameters, const std::string& requestContent) const
         {
            auto taskList = m_taskManager->getAllTasks();
         ...
         shared::CDataContainer CTask::addTask(const std::vector<std::string>& parameters, const std::string& requestContent) const
         {
            try
            {
               auto type = shared::CDataContainer(requestContent).get<std::string>("type");

               auto t = task::CTaskFactory::createTask(type);
         ...
         shared::CDataContainer CTask::serialize(boost::shared_ptr<task::IInstance> instance)
         {
